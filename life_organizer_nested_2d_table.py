  ...
  while True:
    ...
    f"{colors('reset')}NUM     {colors('blue')}TASK                {colors('yellow')}DUE                {colors('magenta')}PRIORITY{colors('reset')}"
      ...
      while True:
        ...
        if len(due_) > 8 or len(due_) < 8:
          print()
          print(f"{colors('red')}Please follow correct format!! (mmddyyyy){colors('reset')}")
          print()
          continue
        else:
          new1 = due_[:2] + "/" + due_[2:]
    ...
    f"{colors('reset')}NUM     {colors('blue')}TASK                {colors('yellow')}DUE                {colors('magenta')}PRIORITY{colors('reset')}"
      ...
      while True:
        ...
        if priority.lower().strip() != 'low' and priority.lower().strip() != 'medium' and priority.lower().strip() != 'high':
          print()
          print(f"{colors('red')}Error, Please try again!{colors('reset')}")
          print()
          continue
      ...
      print()
      while True:
        again_1 = input(f"{colors('red')}Looks like this wasn't on the list! Try again?(y/n)> {colors('reset')}")
...
def remover():
  ...
  if int(which) in range(len(main_list)):
    ...
    print()
    print(f"{colors('red')}Looks like this wasn't on the list!{colors('reset')}")
    print()
    while True:
